...
- **Recurring tasks**: `Task.Recurrence string` (JSON `recurrence,omitempty`) holds a rule in one of four forms — `monthly:N` (1..31, clamped to month-end for short months), `weekly:<day>` (accepts three-letter `mon|tue|...|sun`, full names `monday|...|sunday`, or numeric `1..7` with Mon=1, all case-insensitive), `workdays` (Mon-Fri), or `days:N` (N days after completion). Parsing and next-date math live in `internal/recurrence/` as a pure package (`Parse(string) (Rule, error)`, `Rule.Next(time.Time) time.Time`, `Rule.String()`); `Rule.String()` emits the canonical form (`weekly:mon`, `workdays`) so alias inputs round-trip through edit to canonical storage. Spawn happens only in `cmd/done.go`: when a completed task has a non-empty `Recurrence`, a new task is created with a fresh ULID, copied Title/Body/Source/Recurrence, tags minus `active`, `Schedule = Rule.Next(now)`, and both files are committed in one git commit with message `"done: <title> (recurring, next YYYY-MM-DD)"`. Bidirectional cross-references: the new task gets a `"Spawned from <old-ULID>"` note, the old task gets a `"Spawned follow-up: <new-ULID> (scheduled YYYY-MM-DD)"` note. Stop semantics are edit-based — clearing `Recurrence` (CLI `--recur ""` or TUI YAML edit) before completing prevents the next spawn; there is no SeriesID or series concept. If a stored recurrence fails to parse at done time (hand-edited JSON), the CLI warns to stderr and skips the spawn while still completing the task. CLI surface: `--recur` flag on `add` and `edit` (validated via `recurrence.Parse` at invocation time); TUI surface: a third input `addFocusRecur` in the add modal (edit flows through YAML automatically).
