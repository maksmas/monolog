...
- [x] Move the plan file: done (archived in finalize step).
...
- **Slack polling can race other TUI mutations on git index lock** — the `gitOpInFlight` flag coordinates only the `s`/`git.Sync` path with slack ticks. Other mutations (`d` done, `e` edit, `r` retag, `t` reschedule, `c` grab-commit, `x` delete, `a` toggle-active, `u`/`ctrl+z`/`ctrl+y` undo/redo) dispatch `git.AutoCommitSHA` in goroutines without the flag. A tick-driven ingest arriving mid-mutation can surface `fatal: Unable to create '.git/index.lock'` in the status bar. Window is narrow (Slack Tier-3 polling is fast), impact is a transient error rather than data loss (ingest commit fails cleanly, items remain saved in Slack and will retry on next poll). Broader mitigation (process-level mutex in `internal/git`) is out of scope for v1.
