...
func newEmailSyncCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// email.Sync commits its own batch, and it lives inside
			// internal/email, which must not import internal/config — so the
			// push stays here in the caller, below the output, like every
			// other mutation command. A zero-created sync made no commit (and
			// an err return above means the commit itself failed), so there is
			// nothing to push in either case.
			if res.Created > 0 {
